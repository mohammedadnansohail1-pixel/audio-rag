...
class IngestionPipeline:
    ...
    def ingest(
        ...
        logger.info(f"Ingesting: {audio_path.name} → collection={resolved_collection}")
        ...
        try:
            ...
            # Add source + user metadata to all chunks
            for chunk in chunks:
            ...
            # Store with collection override for multi-tenancy
            ...
            logger.info(
                ...
                f"{len(speakers)} speakers, {duration:.1f}s duration → {resolved_collection}"
            )
